...
(function () {
  ...
  var MACARONS = [
    { name: 'الفراولة',                  flavor: 'Fraise',               cat: 'fruits',    price: 120, shell: '#E8B4B8', cream: '#F2D9DC', photo: ((window.__resources && window.__resources['macRose']) || 'images/m-rose.webp') },
    { name: 'الفستق الحلبي',        flavor: "Pistache d'Alep",      cat: 'noix',      price: 180, shell: '#A7B89A', cream: '#CBD6BE', photo: ((window.__resources && window.__resources['macPistachio']) || 'images/m-pistachio.webp') },
    { name: 'الشوكولاتة السوداء',     flavor: 'Chocolat Noir 65%',    cat: 'chocolat',  price: 150, shell: '#5C3A2E', cream: '#8A5E48', photo: ((window.__resources && window.__resources['macChocolate']) || 'images/m-chocolate.webp') },
    { name: 'موز بالشوكولاتة',         flavor: 'Banane Chocolat',      cat: 'chocolat',  price: 120, shell: '#D4B896', cream: '#ECDABA', photo: ((window.__resources && window.__resources['macBanana']) || 'images/m-banana.webp') },
    { name: 'فراولة بالتوت',           flavor: 'Fraise Framboise',     cat: 'fruits',    price: 140, shell: '#B8475C', cream: '#D67E8E', photo: ((window.__resources && window.__resources['macStrawberry']) || 'images/m-strawberry.webp') },
    { name: 'الليمون',                  flavor: 'Citron',               cat: 'fruits',    price: 120, shell: '#E8C547', cream: '#F2DE8A', photo: ((window.__resources && window.__resources['macLemon']) || 'images/m-lemon.webp') },
    { name: 'جوز الهند والأناناس',    flavor: 'Coco Ananas',          cat: 'fruits',    price: 120, shell: '#F2D04B', cream: '#F7E580', photo: ((window.__resources && window.__resources['macCoconut']) || 'images/m-coconut.webp') },
    { name: 'البندق والبرالين',       flavor: 'Noisette Praliné',     cat: 'noix',      price: 180, shell: '#B79268', cream: '#D2B48C', photo: ((window.__resources && window.__resources['macPecan']) || 'images/m-pecan.webp') },
    { name: 'الفانيلا بالكراميل',     flavor: 'Vanille Caramel',      cat: 'classique', price: 140, shell: '#EAD9B0', cream: '#F4E9CC', photo: ((window.__resources && window.__resources['macVanilla']) || 'images/m-vanilla.webp') },
    { name: 'كرز بالشوكولاتة',         flavor: 'Cerise au Chocolat',   cat: 'fruits',    price: 140, shell: '#A8253A', cream: '#C9434E', photo: ((window.__resources && window.__resources['macCherry']) || 'images/m-cherry.webp') },
    { name: 'جوز الهند بالشوكولاتة',  flavor: 'Coco Chocolat',        cat: 'chocolat',  price: 120, shell: '#A9794E', cream: '#C99E72', photo: ((window.__resources && window.__resources['macCoconutChoc']) || 'images/m-coconut-choc.webp') },
    { name: 'برتقال',                       flavor: 'Orange',               cat: 'fruits',    price: 120, shell: '#F0A040', cream: '#F7C580', photo: ((window.__resources && window.__resources['macOrange']) || 'images/m-orange.webp') }
  ];
  ...
  var dShowCustomQty = false;

  function renderQtyInput() {
    var host = $('.di-boxes', detailEl);
    host.innerHTML = '';
    var isCustom = dBoxSize > 20 || dShowCustomQty;

    /* preset quick-pick + ">20" toggle */
    var presetRow = el('div', 'di-qty-presets');
    QTY_PRESETS.forEach(function (n) {
      var b = el('button', 'di-qp' + (!isCustom && dBoxSize === n ? ' on' : ''));
      b.type = 'button';
      b.textContent = n;
      b.addEventListener('click', function () { dShowCustomQty = false; setBoxSize(n); });
      presetRow.appendChild(b);
    });
    var autreBtn = el('button', 'di-qp di-qp-autre' + (isCustom ? ' on' : ''));
    autreBtn.type = 'button';
    autreBtn.textContent = '>20';
    autreBtn.addEventListener('click', function () {
      dShowCustomQty = true;
      if (dBoxSize <= 20) setBoxSize(21); else renderQtyInput();
    });
    presetRow.appendChild(autreBtn);
    ...
    if (isCustom) {
      ...
      row.innerHTML =
        ...
        '<input type="number" class="di-qty-inp" min="21" value="' + dBoxSize + '" />' +
        '<button type="button" class="di-qty-btn">+</button>';
      host.appendChild(row);
      var inp = row.querySelector('.di-qty-inp');
      row.querySelectorAll('.di-qty-btn')[0].addEventListener('click', function () { setBoxSize(Math.max(21, dBoxSize - 1)); });
      row.querySelectorAll('.di-qty-btn')[1].addEventListener('click', function () { setBoxSize(dBoxSize + 1); });
      inp.addEventListener('change', function () { setBoxSize(Math.max(21, Math.floor(+inp.value) || 21)); });
      inp.addEventListener('input',  function () { var v = Math.floor(+inp.value); if (v >= 21) setBoxSize(v); });
    }
  ...
  function fillDetail(idx) {
    ...
    DMODE = 'single';
    dShowCustomQty = false;
    if (dBoxSize > 20) dBoxSize = 6;
